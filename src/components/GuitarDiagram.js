...
export default function GuitarDiagram({ shape, name, posLabel }) {
  if (!shape) {
    return (
  ...
  // 프렛 x좌표
  const fretX = f => LEFT + (f - startFret + 0.5) * DX;
    ...
    <Svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
      ...
      {shape.frets.map((f, i) => {
        ...
        if (f === 0) {
          // 오픈 원
          return (
            <Circle key={i} cx={LEFT - 14} cy={y} r={5}
        ...
        return (
          <React.Fragment key={i}>
            ...
            <SvgText x={x} y={y} textAnchor="middle"
              dominantBaseline="middle" fill="#111" fontSize={9} fontWeight="bold">
              {shape.fingers[i] || ''}
            </SvgText>
          </React.Fragment>
        );
