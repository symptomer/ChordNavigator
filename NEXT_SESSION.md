...
### ✅ 앱 밖 작업도 끝남 (2026-07-25 12차 후반)
1. **Worker 배포 완료** — Version `3a62b9f9`. 배포 후 엔드포인트 4종 검증(필수값 누락 400 / 새 형식 403 not_premium / GPS 경로 / 구버전 한글 파라미터 하위호환). ⚠️**AI가 실제로 그 언어로 답하는지는 프리미엄 계정이 있어야 확인 가능** → TestFlight에서 사장님이 확인.
2. **웹 개인정보처리방침 정정 완료** — 라이브 페이지에서 Google(Gemini)·Cloudflare 표기 확인. '별도의 서버를 운영하지 않으며'도 사실과 달라 정정(실제로는 중계 Worker + KV에 앱 사용자 ID별 일일 카운터 48시간).
3. **ASC 메타데이터 10개 언어 입력 완료** — `asc/metadata.json` + `asc/push-metadata.js --apply`로 버전 1.0.3 생성 후 반영, API로 되읽어 확인.
4. **빌드·업로드 완료** — 1.0.3 (24). ⚠️**EAS 무료플랜 iOS 빌드 이번 달 소진**(8/1 리셋) → 메트로눈 로컬빌드 체인 재사용(`~/.pruby` + `~/.gems34` + `~/.node20`, 레시피는 project_metronome.md).
5. **시뮬레이터 레이아웃 검증 완료**(사장님이 "언어를 봐도 모른다"고 해서 대신 봄) — iPhone 17 Pro에 기기 언어 de/ru로 홈·설명서·코드탭·기법제안·페이월 확인. **2단 비교표는 열이 좁아 한 단어 16자↑면 단어 중간에서 끊긴다** → ru `последовательностей`, de `KI-GPS-Routenvorschläge` 두 개 축약(커밋 `d852ada`). 전 키 검사로 다른 언어엔 같은 문제 없음 확인.

### ⏭️ 진짜 남은 것
1. 🔴 **심사 제출 버튼** — ASC 1.0.3에서 사장님이 직접. (빌드 연결·메타데이터는 다 돼 있음)
2. 🔴 **TestFlight 실기기 확인 1가지** — 프리미엄이 있어야만 되는 항목이라 클로드가 못 봄: 코드탭 **GPS 루트** 카드가 뜨는지 / 분석탭 **✦ AI 분석**이 답하는지.
3. **ASC 개인정보 답변**(웹 폼) — 방침 본문은 고쳤지만 ASC의 '앱이 수집하는 개인정보' 답변은 미확인.
4. (선택) 안드로이드 — 다국어는 코드 공통이라 그대로 적용됨.
